...
export function loadProfile(cfg) {
  if (!existsSync(cfg.profilePath))
    throw new Error(`supplier profile not found at ${cfg.profilePath} — create it first (see setup guide)`);
  const p = JSON.parse(readFileSync(cfg.profilePath, "utf8"));
...
export function saveProfile(cfg, profile) {
  writeFileSync(cfg.profilePath, JSON.stringify(profile, null, 2) + "\n");
}
